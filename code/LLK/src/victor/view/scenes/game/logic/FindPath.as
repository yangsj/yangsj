package victor.view.scenes.game.logic
{
	...
	public class FindPath
	{
		...
		private var _vecFirst:Vector.<IItem>; // 记录未发生拐点的节点
		private var _vecSecond:Vector.<IItem>; // 记录第一次拐点后的节点
		private var _vecThird:Vector.<IItem>; // 记录第二次拐点后的节点
		...
		private var _startItem:IItem; // 起始节点
		private var _endItem:IItem; // 结束节点
		private var _map:Vector.<Vector.<IItem>>;
		...
		public function initMap( rows:uint, cols:uint, map:Vector.<Vector.<IItem>> ):void
		{
			_rows = rows;
			_cols = cols;
			_map = map;
		...
		public function seek( startItem:IItem, endItem:IItem ):Vector.<IItem>
		{
			_startItem = startItem;
			_endItem = endItem;

			// set init vars
			_seekGroups = new Vector.<Vector.<IItem>>();
		...
		{
			var item:IItem = _map[ _s_rows ][ col ];
			_vecFirst.push( item );
			if ( item.isReal )
			{
		...
		{
			var item:IItem = _map[ row ][ _s_cols ];
			_vecFirst.push( item );
			if ( item.isReal )
			{
				if ( item == _endItem ) // 是结束节点 
