package net.yang.code
{
	...
	public class TestMain
	{
		...
		// 记录已检查过的可移除的对象
		private var _checkRemovedObjectStrArr:Array;
		// 记录需要移动的对象
		...
		*/
		private function initVars():void
		{
			...
			_checkRemovedObjectStrArr= new Array();
		...
		*/
		private function initMap():void
		{
			for (var i:int = 0; i < ROW_HORIZONTAL * ROW_VERTICAL; i++)
			{
				...
				_disObj.x = _tx;
				_disObj.y = _ty;
				_disObj.rowV = _disObj.rowStartV = _RV;
				_disObj.rowH = _disObj.rowStartH = _RH;
		...
		*/
		private function checkRemoveObj($rv:int, $rh:int):void
		{
			...
			var _arrToString:String = _checkRemovedObjectStrArr.toString();
			if (_arrToString.search(_checkStr) == -1)
			{
				_checkRemovedObjectStrArr.push(_checkStr);
		...
		*/
		...
		private function updateDataPool():void
		{
				...
				if (_everyPointObjectArr[_top1])
				{
					...
					if (baseMC)
					{
						...
						if (! boo)
						{
							...
							baseMC.moveRowV +=  1;
							if (baseMC.moveRowV + baseMC.rowV > 9)
							{
		...
		*/
		private function clickAfterClearVars():void
		{
			...
			while (_checkRemovedObjectStrArr.length > 0)
			{
				_checkRemovedObjectStrArr[_checkRemovedObjectStrArr.length - 1] = null;
				_checkRemovedObjectStrArr.pop();
			}
			_canRemoveObjectArr = [];
			_checkRemovedObjectStrArr = [];
		}
