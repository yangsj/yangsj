package net.vyky.code.regular
{
	...
	/**
	 * 说明：InciseDisplayObject 切割显示对象
	 * @author Victor
	 * @email acsh_ysj@163.com
	 * 2012-3-7
	 */

	public class VInciseDisplayObject
	{
		...
		public function VInciseDisplayObject()
		{
		}
		 ...
		 *
		 */
		public function cutImage($photoName:DisplayObject, $numX:uint=0, $numY:uint=0, $apart:Number=0, $type:String="sprite", $isDrag:Boolean=false, $container:DisplayObjectContainer=null):void
		{
				...
				for (var j:uint=0; j < ($numX + 1); j++)
				{
					for (var k:uint=0; k < ($numY + 1); k++)
					{
						...
						if (array[counter] == null)
						{
							...
							spr.graphics.beginFill(0xfff000, 0);
							spr.graphics.drawRect(0, 0, arrXY[counter][0], arrXY[counter][1]);
		...
		private function listenerDrag(container:DisplayObjectContainer=null):void
		{
			...
			function startDragItem(e:Event):void
			{
			...
			function stopDragItem(e:Event):void
			{
				//停止拖拽
				if (String(describeType(e.target).@name) == "flash.display::Sprite")
				{
