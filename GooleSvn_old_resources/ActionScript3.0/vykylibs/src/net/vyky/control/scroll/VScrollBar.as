package net.vyky.control.scroll
{
	...
	public class VScrollBar
	{
		...
		/** 目标对象的滑动范围 */
		private var targetValidRect:Number;
		 ...
		 * 
		 */
		public function VScrollBar($target:DisplayObject=null, $rect:Rectangle=null, $scrollUI:DisplayObjectContainer=null, $direction:String='vertical')
		{
			target    = $target;
			rect	  = $rect;
			scrollUI  = $scrollUI;
		...
		public function initialize():void
		{
			...
			bar_startX 		= scrollBar.x;
			bar_startY 		= scrollBar.y;
			
			bar_rectangle_X = scrollSlot.width  - scrollBar.width;
			bar_rectangle_Y = scrollSlot.height - scrollBar.height;
			setTargetPoint();
			if (stage)
			{
		...
		{
			if (VScrollBarDirection.HORIZONTAL == direction)
			{
				return int((scrollBar.x - scrollBarThickness) / barValidMoveLength * maxScrollPosition);
			}
			else if (VScrollBarDirection.VERTICAL == direction)
			{
		...
		public function set scrollCurrentPosition(value:Number):void 
		{
			...
			if (VScrollBarDirection.HORIZONTAL == direction) scrollBar.x = temp; 
			else if (VScrollBarDirection.VERTICAL == direction) scrollBar.y = temp;
		...
		public function set scrollBar(value:Sprite):void 
		{
			...
			if (_scrollBar)
			{
				...
				scrollBarLength = direction == VScrollBarDirection.HORIZONTAL ? value.width : value.height;
			}
		}
		...
		public function set scrollSlot(value:DisplayObject):void 
		{
			...
			if (_scrollSlot)
			{
				scrollSlotLength = direction == VScrollBarDirection.HORIZONTAL ? value.width : value.height;
			}
		}
		...
		private function JudgeScrollExistent():void
		{
			if ( scrollBar == null || scrollSlot == null)
			{
				...
				if ( direction == VScrollBarDirection.HORIZONTAL)
				{	
				...
				else if (direction == VScrollBarDirection.VERTICAL)
				{
				...
				{
					scrollContainer.x = (direction == VScrollBarDirection.HORIZONTAL) ? target.x : target.x + rect.width;
					scrollContainer.y = (direction == VScrollBarDirection.HORIZONTAL) ? target.y + rect.height : target.y;
				}
			...
			else
			{
				...
				scrollBarThickness = (direction == VScrollBarDirection.HORIZONTAL) ? scrollSlot.x : scrollSlot.y;
			}
		}
		...
		{
			if (direction == VScrollBarDirection.HORIZONTAL) targetValidRect = target.width - rect.width;
			else targetValidRect = target.height - rect.height;
			
			scrollContainer.visible = targetValidRect > 0 ? true : false;
			if (targetValidRect<=0)
			{
				if (VScrollBarDirection.HORIZONTAL == direction) target.x = _targetStartPointX; 
				else if (VScrollBarDirection.VERTICAL == direction) target.y = _targetStartPointY;
				return;
			}
			...
			if (VScrollBarDirection.HORIZONTAL == direction) target.x = _targetStartPointX - temp; 
			else if (VScrollBarDirection.VERTICAL == direction) target.y = _targetStartPointY - temp; 
		}
		...
		private function destroy():void
		{
			...
			if (scrollBar) TweenMax.killTweensOf(scrollBar);
			if (_maskRes)
			{
				if (_maskRes.parent) _maskRes.parent.removeChild(_maskRes);
				_maskRes = null;
			}
			if (_isNeedFilter == false && scrollContainer)
			{
				if (scrollContainer.parent) scrollContainer.parent.removeChild(scrollContainer);
			...
			if (scrollUI)scrollUI.visible = true;
		...
		private function scrollBarHandler(e:MouseEvent):void 
		{
			...
			else if (e.type == MouseEvent.MOUSE_WHEEL)
			{
				...
				if (direction == VScrollBarDirection.VERTICAL)
				{
					bar_move_y = scrollBar.y;
					bar_move_y -= e.delta * 10;
				...
				else if (direction == VScrollBarDirection.HORIZONTAL)
				{
					bar_move_x = scrollBar.x;
					bar_move_x += e.delta * 5;
