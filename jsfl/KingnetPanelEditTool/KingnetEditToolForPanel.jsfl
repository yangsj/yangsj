...
function startQuery(item)
{
	...
	if (item.linkageExportForAS)
	{
		...
		importName +=  "\n\timport pet.game.panels.core.BasePanel;";
						...
						for (var k = 0; k < elements.length; k++)
						{
							...
							if (item2.name)
							{
								...
								if (boo)
								{
									...
									if (strName != "_Close" && strName != "mc_drag" && strName != "_TitleBar")
									{
		...
		getFun +=  "\n\t\t\treturn \"" + linkageClassName + "\";\n\t\t}";

		getFun +=  "\n\t\toverride public function get resName():String\n\t\t{";
		getFun +=  "\n\t\t\treturn \"" + fileName + "\";\n\t\t}";

		textToWrite +=  packageName;
		textToWrite +=  importName;
		textToWrite +=  "\n\n\t/**\n\t * 资源信息：<br>" + resourceInformationDes + "\n\t * ……" + documentsDes + "\n\t * @author " + authorName + "\n\t */";
		textToWrite +=  "\n\tpublic class " + className + " extends BasePanel\n\t{";
