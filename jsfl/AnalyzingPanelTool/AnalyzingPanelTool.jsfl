				...
				if (j == frames[j].startFrame)
				{
					...
					for (var k = 0; k < elements.length; k++)
					{
						...
						if(item2.libraryItem)
						{
							...
							{
								var strImp = libItem.linkageClassName;
								if (strImp.lastIndexOf(".") > -1)
								{
									var temparrStr = importArr.toString();
									if (temparrStr.search("import " + libItem.linkageClassName + ";") == -1)
									{
										importArr.push("\n\timport " + libItem.linkageClassName + ";");
									}
								}
								/*strImp = strImp.substr(strImp.lastIndexOf(".")+1);*/
								typeArr.push(strImp);
							}
							else
							{
								...
								var temparrStr = importArr.toString();
								if (temparrStr.search("import flash.display.DisplayObject;") == -1)
								{
									importArr.push("\n\timport flash.display.DisplayObject;");
								}
								typeArr.push("DisplayObject");
							}
						}
						else if (item2.elementType == "text")
						{
							...
							if (temparrStr.search("import flash.text.TextFormat;") == -1)
							{
								...
								if (readFonts == "false")
								{
									importArr.push("\n\timport net.jt_tech.diamond.code.text.CharacterStyle;");
								}
							}	
...
function analysisCase()
{
	...
	for (var i = leng; i > -1; i--)
	{
		...
		if (clsArr[i])
		{
			...
			{
				var clsss = "this.createUIItem(\"" + clsArr[i] + "\") as " + typeArr[i] + ";";
				cases += "\n\t\t\t\t_" + strName + "	= " + clsss;
			}
...
function startingStr()
{
	...
	imports += "\n\tpublic class " + className + " extends JTPanelBase";
	imports += "\n\t{";
